...
const QUESTIONS = [
  ...
  { q: "인터넷에서 이상한 문자가 왔는데 눌러도 될까요?", type: "in" },
  { q: "경찰인데 계좌를 확인해야 한다고 전화가 왔어요, 어떻게 하죠?", type: "in" },
  { q: "주민등록등본을 인터넷으로 뗄 수 있나요?", type: "in" },
  { q: "동네에 있는 기계로 서류를 뽑을 수 있다던데 그게 뭐예요?", type: "in" },
  { q: "디지털배움터에서 배우는 내용에는 어떤 게 있어요?", type: "in" },
  { q: "디지털배움터는 정부에서 하는 건가요?", type: "in" },
  { q: "장애인도 디지털배움터 교육 받을 수 있나요?", type: "in" },
  { q: "AI 안전 교육이라는 게 딥페이크도 알려주나요?", type: "in" },
  { q: "디지털배움터 관련 문의는 과학기술정보통신부에도 할 수 있나요?", type: "in" },
  { q: "우리 동네 주민센터에서도 디지털 교육을 받을 수 있나요?", type: "in" },
  { q: "디지털배움터 교육은 몇 시간짜리예요?", type: "ambiguous" },
  { q: "디지털배움터 홈페이지 주소가 정확히 뭐예요?", type: "ambiguous" },
  { q: "요즘 인기있는 드라마 추천해주세요", type: "out" },
  { q: "고혈압에 좋은 음식이 뭐예요?", type: "out" },
  { q: "지하철 요금이 얼마예요?", type: "out" },
];
...
async function judge(query, answer, hits) {
  ...
  const r = await fetch(`${OLLAMA}/api/chat`, {
    method: "POST", headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ model: "qwen3.5:2b", stream: false, think: false, format: "json", options: { temperature: 0 }, messages: [{ role: "system", content: sys }, { role: "user", content: user }] }),
  });
...
async function main() {
  ...
  await writeFile(new URL("./eval-results-4b.json", import.meta.url), JSON.stringify(results, null, 2));
